...
│   ├── backend/
│   │   ├── config.ts                # AppConfig 配置解析（环境变量 → 结构化配置）
│   │   ├── app-runtime.ts           # AppRuntime：依赖注入容器 + 消息发送入口
│   │   ├── worker-runtime.ts        # WorkerRuntime：Electron Worker 模式的 API 入口
│   │   ├── job-registry.ts          # JobRegistry：后台任务注册中心（并发控制）
│   │   ├── media-manager.ts         # 媒体资源管理器（音频/图片/文件）
│   │   ├── db/database.ts           # SQLite 仓库，6 张表，5 个索引
│   │   ├── graph/
│   │   │   ├── chat-graphs.ts       # 单聊 LangGraph 图（7 节点）
│   │   │   └── group-coordinator.ts # 群聊多 Agent 协调器
│   │   └── services/
│   │       ├── characters/character-service.ts      # 角色配置加载与解析
│   │       ├── llm/deepseek-service.ts              # DeepSeek LLM 服务（流式 + 结构化）
│   │       ├── memory/memory-service.ts             # 三层记忆系统（L1/L2/L3）
│   │       ├── stream/sse-service.ts                # SSE 流式服务（独立 HTTP 服务器）
│   │       ├── tts/tts-service.ts                   # TTS 语音合成服务
│   │       └── es/
│   │           ├── elasticsearch-service.ts          # Elasticsearch 客户端封装
│   │           └── bge-m3-embedding-service.ts       # bge-m3 Embedding 服务
│   └── renderer/
│       ├── main.tsx                 # 前端入口
│       ├── App.tsx                  # 根组件 + Provider 层级（Context 嵌套）
│       ├── types.ts                 # 前端专用类型
│       ├── api/client.ts            # API 客户端（fetch 封装）
│       ├── hooks/useChatStream.ts   # SSE 流式消费 Hook（EventSource）
│       ├── utils/avatar.ts          # 头像路径解析工具
│       ├── components/
│       │   ├── ChatWorkspace.tsx    # 聊天工作区容器
│       │   ├── MessageList.tsx      # 消息列表（虚拟滚动）
│       │   ├── MessageBubble.tsx    # 单条消息气泡（文本/图片/音频）
│       │   ├── MessageComposer.tsx  # 消息输入框（文本+附件上传）
│       │   └── AudioPlayer.tsx      # 语音播放器（播放/重试/刷新）
│       ├── pages/
│       │   ├── CharacterListPage.tsx    # 角色列表页
│       │   ├── SingleChatPage.tsx       # 单聊页面
│       │   ├── GroupChatCreatePage.tsx  # 群聊创建页
│       │   ├── GroupChatPage.tsx        # 群聊页面
│       │   └── SettingsPage.tsx         # 系统设置页
│       └── context/
│           ├── BootstrapContext.tsx  # 角色/会话初始化数据
│           ├── ViewContext.tsx       # 视图路由 + 聊天 CRUD 操作
│           └── ChatContext.tsx       # 消息状态管理与分发
├── scripts/
